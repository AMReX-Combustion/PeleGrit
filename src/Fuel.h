...
{
private:
  const double nasapolyair_[5] = {1052.71406, -0.3745355, 8.361477e-4,
                                  -3.32111e-7, -4.683905e-11};

  const double nasapolydecane_[6] = {3.19882239e1,    4.77244922e-2,
                                     -1.62276391e-5,  2.50963259e-9,
                                     -1.45215772e-13, -4.66392840e4};
...
public:
  ...
  void updateh() { href_ = calcEnthalpy(Tref_, nasapolydecane_, MW_decane_); };
  ...
  double calcEnthalpy(const double T, const double *nasapoly, const double mw)
  {
    return (nasapoly[0] + nasapoly[1] * T / 2.0 + nasapoly[2] * T * T / 3.0 +
            nasapoly[3] * T * T * T / 4.0 +
            nasapoly[4] * std::pow(T, 4.0) / 5.0 + nasapoly[5] / T) *
           R_ / mw * T;
  }
};
#endif
