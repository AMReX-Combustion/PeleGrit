...
public:
  ...
  void initializeGrit();
  void finalizeGrit();
  void advanceSprays(const long int np, const double dt, const double *dx,
                     const int DIM, const int NX, const int NY, const int NZ,
                     const int rhogindex, const int rhogUgindex,
                     const int rhogegindex, const int Tgindex,
                     const int Ygindex, bool do_move, const int sNX,
                     const int sNY, const int sNZ);
  int getIndex(const int small_n, const int small_NX, const int small_NY,
               const int small_NZ, const int DIFFNX, const int DIFFNY,
               const int DIFFNZ);
};
#endif
