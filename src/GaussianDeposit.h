...
public:
  ...
  static void depositVectr3(const int NX, const int NY, const int NZ,
                            const int NV, const int NP, Yarn::VectorFieldType F,
          ...
          for (int kk = -NH; kk < 2 + NH; kk++)
          {
            for (int jj = -NH; jj < 2 + NH; jj++)
            {
              for (int ii = -NH; ii < 2 + NH; ii++)
              {
                ...
                double delz = kk - rz;
                filtersum +=
                    exp(-0.5 * (delx * delx + dely * dely + delz * delz));
              }
            }
          }
            ...
            for (int jj = -NH; jj < 2 + NH; jj++)
            {
              for (int ii = -NH; ii < 2 + NH; ii++)
              {
                ...
                double filtercoeff =
                    exp(-0.5 * (delx * delx + dely * dely + delz * delz)) /
                    filtersum;
                size_t nn = (kz + kk) * (NX + 1) * (NY + 1) +
                            (jy + jj) * (NX + 1) + (ix + ii);
                for (int l = 0; l < NV; l++)
                  F_access(nn, l) += filtercoeff * P(n, l);
              }
            }
          }
        });
  ...
  /* ---------------------------------------------------------------------- */
  // 2-D
  static void depositVectr3(const int NX, const int NY, const int NV,
                            const int NP, Yarn::VectorFieldType F,
                            Dust_Pele::Vectr3PointType loc,
                            Dust_Pele::HealthPointType state,
                            Dust_Pele::Vectr3PointType P)
  {
        ...
        NP, KOKKOS_LAMBDA(const size_t &n) {
          ...
          for (int jj = -NH; jj < 2 + NH; jj++)
          {
            for (int ii = -NH; ii < 2 + NH; ii++)
            {
              ...
              double dely = jj - ry;
              double filtercoeff =
                  exp(-0.5 * (delx * delx + dely * dely)) / filtersum;
  ...
  static void depositScalar(const int NX, const int NY, const int NP,
                            Yarn::ScalarFieldType F,
                            Dust_Pele::Vectr3PointType loc,
                            Dust_Pele::HealthPointType state,
                            Dust_Pele::ScalarPointType P)
  {
    assert(F.dimension_0() == (NX + 1 + 2 * NH) * (NY + 1 + 2 * NH));
  ...
  /* ---------------------------------------------------------------------- */
  // 3-D
  static void depositScalar(const int NX, const int NY, const int NZ,
                            const int NP, Yarn::ScalarFieldType F,
                            Dust_Pele::Vectr3PointType loc,
                            Dust_Pele::HealthPointType state,
                            Dust_Pele::ScalarPointType P)
  {
