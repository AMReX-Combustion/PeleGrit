...
{
  Kokkos::initialize();
  ptr_spray = std::unique_ptr<PeleSprayParticle[]>(
      new PeleSprayParticle[Spray_Pele::NDUST]);
  std::cout << "Grit is initialized" << std::endl;
}

void GritPeleSprayInterface::advanceSprays(
    const long int NP, const double dt, const double *dx, const int DIM,
    const int NX, const int NY, const int NZ, const int rhogindex,
    const int rhogUgindex, const int rhogegindex, const int Tgindex,
    const int Ygindex, bool do_move, const int sNX, const int sNY,
    const int sNZ)
{
  ...
  if (DIM == 3)
  {
    LagrangeInterp<NH>::interpolateVector(NX - 1, NY - 1, NZ - 1, DIM, NP, Ug,
                                          grit_spray.loc, grit_spray.state,
                                          Ugpar);
    LagrangeInterp<NH>::interpolateScalar(NX - 1, NY - 1, NZ - 1, NP, rhog,
                                          grit_spray.loc, grit_spray.state,
                                          rhogpar);
    LagrangeInterp<NH>::interpolateScalar(NX - 1, NY - 1, NZ - 1, NP, Tg,
                                          grit_spray.loc, grit_spray.state,
                                          Tgpar);
    LagrangeInterp<NH>::interpolateScalar(NX - 1, NY - 1, NZ - 1, NP, Yg,
                                          grit_spray.loc, grit_spray.state,
                                          Ygpar);
  }
  else
  {
    LagrangeInterp<NH>::interpolateVector(
        NX - 1, NY - 1, DIM, NP, Ug, grit_spray.loc, grit_spray.state, Ugpar);
    LagrangeInterp<NH>::interpolateScalar(
        NX - 1, NY - 1, NP, rhog, grit_spray.loc, grit_spray.state, rhogpar);
    LagrangeInterp<NH>::interpolateScalar(
        NX - 1, NY - 1, NP, Tg, grit_spray.loc, grit_spray.state, Tgpar);
    LagrangeInterp<NH>::interpolateScalar(
        NX - 1, NY - 1, NP, Yg, grit_spray.loc, grit_spray.state, Ygpar);
  }
  ...
  Kokkos::parallel_for(
      NP, KOKKOS_LAMBDA(const size_t &n) {
        ...
        hvatTd(n) = b0 + b1 * Td / 2.0 + b2 * Td * Td / 3.0 +
                    b3 * Td * Td * Td / 4.0 + b4 * pow(Td, 4.0) / 5.0 + b5 / Td;
        hvatTd(n) *= R / MWf * Td;
        ...
        double Psat =
            atm * exp(hfg(n) * MWf * (1.0 / R) * (1.0 / T_boil - 1.0 / Td));
        ...
        // calc Cp_v
        double Cpair = a0 + a1 * Tv(n) + a2 * Tv(n) * Tv(n) +
                       a3 * Tv(n) * Tv(n) * Tv(n) + a4 * pow(Tv(n), 4);

        Cpvf(n) = (b0 + b1 * Tv(n) + b2 * Tv(n) * Tv(n) +
                   b3 * Tv(n) * Tv(n) * Tv(n) + b4 * pow(Tv(n), 4)) *
                  R / MWf;
  ...
  Kokkos::parallel_for(
      NP, KOKKOS_LAMBDA(const size_t &n) {
        ...
        double mu_air =
            mu_b * pow(Tv(n) / mu_T0, 1.5) * (mu_T0 + mu_S) / (Tv(n) + mu_S);
        double mu_fuel = 1.7083e-8 * Tv(n) + 5.922e-8;
        mu_v(n) = mu_air * (1.0 - Yv(n)) + mu_fuel * Yv(n);

        double denomD = cbrt(208.56) + cbrt(20.1);
        D_v(n) = 1e-3 * pow(Tv(n), 1.75) *
                 sqrt(1.0 / MWa / 1000 + 1.0 / MWf / 1000) / (Pgpar(n) / atm) /
                 denomD / denomD; // cgs
  ...
  Kokkos::parallel_for(
      NP, KOKKOS_LAMBDA(const size_t &n) {
        ...
        magU = sqrt(magU);
        grit_spray.Re(n) =
            rhogpar(n) * magU * grit_spray.dia(n) / (mu_v(n) * 10);
        double dragCoef, drag;
        dragCoef = 24.0 / grit_spray.Re(n) *
                   (1.0 + pow(grit_spray.Re(n), 2.0 / 3.0) / 6.0);
        drag = 0.125 * rhogpar(n) * dragCoef * pi * grit_spray.dia(n) *
               grit_spray.dia(n) * magU;
        // m is also g here
        m(n) = 1.0 / 6.0 * pi * rho_d * grit_spray.dia(n) * grit_spray.dia(n) *
               grit_spray.dia(n);
        for (size_t l = 0; l < DIM; l++)
        {
          rhsu(n, l) = (Ugpar(n, l) - grit_spray.vel(n, l)) * drag;
          grit_spray.vel(n, l) =
              grit_spray.vel(n, l) + rhsu(n, l) / m(n) * part_dt;
        }
      });
  ...
  if (do_move)
  {
    Kokkos::parallel_for(
        NP, KOKKOS_LAMBDA(const size_t &n) {
          ...
          // integrate locations
          grit_spray.loc(n, 0) =
              grit_spray.loc(n, 0) + grit_spray.vel(n, 0) * dt / cellsizex;
          grit_spray.loc(n, 1) =
              grit_spray.loc(n, 1) + grit_spray.vel(n, 1) * dt / cellsizey;
          grit_spray.loc(n, 2) =
              grit_spray.loc(n, 2) + grit_spray.vel(n, 2) * dt / cellsizez;
        });
  }
  ...
  Kokkos::parallel_for(
      NP, KOKKOS_LAMBDA(const size_t &n) {
        ...
        double fm =
            max(Sp_Bm(n) / (log(1 + Sp_Bm(n)) * pow((1 + Sp_Bm(n)), 0.7)), 0.0);
        Shv(n) = 2.0 + (Shv(n) - 2.0) * fm;
        srcm(n) = -pi * rhogpar(n) * D_v(n) * grit_spray.dia(n) * Shv(n) *
                  log(1 + Sp_Bm(n));
  ...
  Kokkos::parallel_for(
      NP, KOKKOS_LAMBDA(const size_t &n) {
        if (grit_spray.state(n) != 1)
          return;
        double Nud = 1.0 + pow(1.0 + grit_spray.Re(n) * Pr_v(n), 1.0 / 3.0) *
                               (max(pow(grit_spray.Re(n), 0.077), 1.0));
        ...
        rhse(n) = (Tgpar(n) - grit_spray.temp(n)) * (lambdav(n) * 1e5) * Nud *
                  pi * grit_spray.dia(n);
  ...
  Kokkos::parallel_for(
      NP, KOKKOS_LAMBDA(const size_t &n) {
        ...
        srce(n) =
            rhse(n) + smudotu + srcm(n) * ((hvatTd(n) * 1e4) + 0.5 * upart2);
      });

  if (DIM == 3)
  {
    GaussianDeposit<NH>::depositVectr3(NX - 1, NY - 1, NZ - 1, DIM, NP, Srcug,
                                       grit_spray.loc, grit_spray.state, srcu);
    GaussianDeposit<NH>::depositScalar(NX - 1, NY - 1, NZ - 1, NP, Srcmg,
                                       grit_spray.loc, grit_spray.state, srcm);
    GaussianDeposit<NH>::depositScalar(NX - 1, NY - 1, NZ - 1, NP, Srceg,
                                       grit_spray.loc, grit_spray.state, srce);
  }
  else
  {
    GaussianDeposit<NH>::depositVectr3(NX - 1, NY - 1, DIM, NP, Srcug,
                                       grit_spray.loc, grit_spray.state, srcu);
    GaussianDeposit<NH>::depositScalar(NX - 1, NY - 1, NP, Srcmg,
                                       grit_spray.loc, grit_spray.state, srcm);
    GaussianDeposit<NH>::depositScalar(NX - 1, NY - 1, NP, Srceg,
                                       grit_spray.loc, grit_spray.state, srce);
  }
...
int GritPeleSprayInterface::getIndex(const int sn, const int sNX, const int sNY,
                                     const int sNZ, const int DIFFNX,
                                     const int DIFFNY, const int DIFFNZ)
{
...
void GritPeleSprayInterface::finalizeGrit() { Kokkos::finalize(); }
